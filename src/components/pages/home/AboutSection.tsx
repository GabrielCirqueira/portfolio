...
export const AboutSection = memo(() => {
  const { ehDispositivoLento, usarEfeitosPesados } = useAnimation()
    ...
    >
      ...
      {usarEfeitosPesados && (
        <>
          ...
          >
            <div
              className={`w-full h-full bg-brand-500/10 rounded-full ${ehDispositivoLento ? 'blur-[60px]' : 'blur-[120px]'}`}
            />
          </motion.div>
          ...
          >
            <div
              className={`w-full h-full bg-blue-600/10 rounded-full ${ehDispositivoLento ? 'blur-[60px]' : 'blur-[120px]'}`}
            />
          </motion.div>
        </>
      )}
